        ...
        <section className="mx-auto grid w-full max-w-5xl gap-4">
          {seasons.map((season, index) => (
            <MotionReveal key={season.id} delay={index * 0.06}>
              <SeasonCard season={season} priority={index === 0} />
            </MotionReveal>
          ))}
        </section>
      </main>
...
function SeasonCard({ season, priority }: { season: (typeof seasons)[number]; priority: boolean }) {
  const locked = season.status === "COMING SOON";
  const className = [
    "glass-panel group relative flex min-h-[150px] overflow-hidden rounded-xl border-purple-500/30 p-4 shadow-[0_0_30px_rgba(139,92,246,0.16)] sm:min-h-[170px] sm:p-5",
    locked
      ? "cursor-not-allowed opacity-[0.72]"
      : "neon-hover hover:scale-[1.006] hover:border-purple-200/70 hover:shadow-[0_0_44px_rgba(139,92,246,0.3)]"
  ].join(" ");
  const content = <SeasonCardContent season={season} priority={priority} locked={locked} />;
  ...
  return (
    <Link href={`/seasons/${season.id}`} className={className}>
      {content}
    </Link>
  );
}

function SeasonCardContent({ season, priority, locked }: { season: (typeof seasons)[number]; priority: boolean; locked: boolean }) {
  return (
    <>
      ...
      {locked ? <div className="pointer-events-none absolute inset-0 bg-black/18" /> : null}
      <div className="relative flex min-w-0 flex-1 flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
        <Image src={season.logo} alt={`${season.title} logo`} width={260} height={160} className="h-auto w-[132px] shrink-0 self-center object-contain drop-shadow-[0_0_24px_rgba(168,85,247,0.38)] transition duration-200 group-hover:scale-[1.025] sm:w-[220px] sm:self-auto" priority={priority} />
...
function StatusBadge({ status }: { status: string }) {
  if (status === "COMING SOON") {
    return (
      <span className="border border-amber-200/45 bg-amber-300/12 px-2.5 py-1 text-[0.65rem] font-black uppercase tracking-[0.16em] text-amber-100 shadow-[0_0_20px_rgba(251,191,36,0.2)]">
        {status}
      </span>
    );
  }
